...
interface TarefaModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (tarefa: { nome: string; descricao: string; urgencia: string }) => void;
}

export const TarefaModal = ({ isOpen, onClose, onSave }: TarefaModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (nome.trim()) {
      onSave({ nome, descricao, urgencia });
      setNome('');
      setDescricao('');
      setUrgencia('normal');
      onClose();
